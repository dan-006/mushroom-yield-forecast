...
# Sampling frequency
sampling_days = (
    df["timestamp"].sort_values().diff().dt.days.dropna()
)
freq = sampling_days.mode()[0]
...
if abs(yield_mean - yield_median) < 0.5:
    ...
    report.append(
        "- Mean and median differ noticeably, suggesting skew in yield."
    )
...
report.append(
    f"- Yield variability is low (CV = {summary.loc['yield_kg', 'cv']}), suggesting relatively consistent mushroom production throughout the year."
)
# Save report
